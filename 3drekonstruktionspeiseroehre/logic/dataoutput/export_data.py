...
class ExportData:
    ...
    def get_data(self, selected_tables):
        try:
            ...
            join_stmt = tables["patients"].outerjoin(tables["visits"], tables["visits"].c.patient_id == tables["patients"].c.patient_id)

            if "previous_therapies" in selected_tables:
                join_stmt = join_stmt.outerjoin(tables["previous_therapies"], tables["patients"].c.patient_id == tables["previous_therapies"].c.patient_id)

            for table in selected_tables:
                if table not in ["patients", "visits", "previous_therapies"]:
                    join_stmt = join_stmt.outerjoin(tables[table], tables[table].c.visit_id == tables["visits"].c.visit_id)
    ...
    def export_csv(data, selected_tables, csv_file_path):
        try:
            with open(csv_file_path, mode='w', newline='', encoding='utf-8') as file:
                ...
                headers_dict = {
                    "patients": ["patient_id", "gender", "ethnicity", "birth_year", "year_first_diagnosis", "year_first_symptoms", "center", "height_cm"],
                    "visits": ["visit_id", "patient_id", "year_of_visit", "visit_type", "therapy_type", "months_after_therapy"],
                    "previous_therapies": ["previous_therapy_id", "patient_id", "therapy", "year", "center"],
                    "eckardt_scores": ["eckardt_id", "visit_id", "dysphagia", "retrosternal_pain", "regurgitation", "weightloss", "total_score"],
                    ...
                    "poems": [
                        ...
                        "visit_id",
                        "procedure_duration",
                        "height_mucosal_incision",
                        "length_mucosal_incision",
                        "length_submuscosal_tunnel",
                        "localization_myotomy",
                        "length_tubular_myotomy",
                        "length_gastric_myotomy",
                    ],
                    "complications": [
                        "complication_id",
                        "visit_id",
                    ...
                    "barium_swallows": [
                        "tbe_id",
                        "visit_id",
                        "type_contrast_medium",
                        "amount_contrast_medium",
                        "height_contrast_medium_1min",
                        "height_contrast_medium_2min",
                        "height_contrast_medium_5min",
                        "width_contrast_medium_1min",
                        "width_contrast_medium_2min",
                        "width_contrast_medium_5min",
                    ],
