...
class VisualizationWindow(QMainWindow):
    ...
    def __init__(self, master_window: MasterWindow, patient_data: PatientData):
        ...
        print(
            f"patient_data.visit_data_dict.items() = Visits: {self.patient_data.visit_data_dict.items()}"
        )
    ...
    def __save_reconstruction_in_db(self):
        ...
        for name, visit_data in self.patient_data.visit_data_dict.items():
            ...
            if (
                not reconstruction
                or reconstruction
                and ShowMessage.to_update_for_visit("3d reconstruction")
            ):
                reconstruction_dict = {
                    "visit_id": visit,
        ...
        try:
            ...
            with open(destination_file_path, "w", newline="") as csv_file:
                ...
                for i, (name, visit_data) in enumerate(self.visits.items()):
                    ...
                    for j in range(len(visit_data.visualization_data_list)):
                        xray_name = visit_data.visualization_data_list[j].xray_minute
                        tubular_metric = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[0]
                        sphincter_metric = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[1]
                        volume_tubular = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[2]
                        volume_sphincter = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[3]
                        max_pressure_tubular = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[4]
                        max_pressure_sphincter = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[5]
                        max_metric_tubular = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[6]
                        max_metric_sphincter = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[7]
                        min_metric_tubular = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[8]
                        min_metric_sphincter = visit_data.visualization_data_list[
                            j
                        ].figure_creator.get_metrics()[9]
                        ...
                        writer.writerow(
                            [
                                visit_name,
                                xray_name,
                                round(np.mean(tubular_metric), 2),
                                round(np.mean(sphincter_metric), 2),
                                round(volume_tubular, 2),
                                round(volume_sphincter, 2),
                                round(max_pressure_tubular, 2),
                                round(max_pressure_sphincter, 2),
                                round(max_metric_tubular, 2),
                                round(max_metric_sphincter, 2),
                                round(min_metric_tubular, 2),
