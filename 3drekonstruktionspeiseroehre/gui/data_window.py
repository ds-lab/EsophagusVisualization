...
class DataWindow(QMainWindow):
    ...
    def __init__(self, master_window: MasterWindow, patient_data: PatientData = PatientData()):
        ...
        self.ui.visit_add_button.clicked.connect(
            self.__visit_add_button_clicked)
        self.ui.visit_delete_button.clicked.connect(
            self.__visit_delete_button_clicked)
        ...
        self.ui.firstdiagnosis_radio.toggled.connect(
            self.__patients_apply_filter)
        self.ui.firstsymptoms_radio.toggled.connect(
            self.__patients_apply_filter)
        ...
        self.ui.endoscopy_upload_button.clicked.connect(
            self.__endoscopy_upload_button_clicked)
    ...
    def __init_ui(self, filter_column: int = 6, filter_expression: str = '.*'):
        ...
        self.patient_tableView.clicked.connect(
            self.__show_selected_patient_data)
    ...
    def __context_menu_patient(self):
        ...
        if self.patient_tableView.selectedIndexes():
            ...
            remove_data.triggered.connect(lambda: self.patient_model.removeRows(
    ...
    def __patient_add_button_clicked(self):
        if self.__validate_patient():
            if self.__patient_exists():
                ...
                self.notify("patient has been created")
            self.__init_ui()
    ...
    def __patients_apply_filter(self):
        if self.ui.patient_id_radio.isChecked():
            ...
            filter_exp = '^' + self.ui.patient_id_field.text()
        ...
        if self.ui.firstdiagnosis_radio.isChecked():
            filter_exp = str(
                self.ui.firstdiagnosis_calendar.date().toPyDate().year)
            self.__init_ui(filter_column=4, filter_expression=filter_exp)
        if self.ui.firstsymptoms_radio.isChecked():
            filter_exp = str(
                self.ui.firstsymptoms_calendar.date().toPyDate().year)
        ...
        if self.ui.center_radio.isChecked():
            ...
            filter_exp = '^' + self.ui.center_text.text()
    ...
    def __init_previous_therapies(self):
        ...
        self.therapy_tableView.setContextMenuPolicy(
            QtCore.Qt.ContextMenuPolicy.CustomContextMenu)
        self.therapy_tableView.customContextMenuRequested.connect(
            self.__context_menu_previous_therapies)
    ...
    def __show_selected_previous_therapy_data(self):
        # Get the indexes of all selected cells
        selected_indexes = self.therapy_tableView.selectedIndexes()
        if selected_indexes:
            # Get the row number of the first selected index
            selected_row = selected_indexes[0].row()
    ...
    def __previous_therapy_add_button_clicked(self):
        if (
            ...
            if self.ui.therapy_year_unknown_checkbox.isChecked():
                ...
                self.previous_therapy_service.create_previous_therapy(
                    therapy_dict)
                self.__init_previous_therapies()
            else:
                therapy_dict = {
    ...
    def __init_visits_of_patient(self):
        ...
        self.visits_tableView.setContextMenuPolicy(
            QtCore.Qt.ContextMenuPolicy.CustomContextMenu)
        self.visits_tableView.customContextMenuRequested.connect(
            self.__context_menu_visit)
    ...
    def __context_menu_visit(self):
        ...
        if self.visits_tableView.selectedIndexes():
            remove_data = menu.addAction("Remove Data")
            remove_data.setIcon(QtGui.QIcon("./media/remove.png"))
            remove_data.triggered.connect(lambda: self.visits_model.removeRows(
                self.visits_tableView.currentIndex()))
        cursor = QtGui.QCursor()
        menu.exec(cursor.pos())
    ...
    def __visit_delete_button_clicked(self):
        if self.visit_exists():
            ...
            QMessageBox.warning(self, "Select Visit to Delete",
                                "Please select a visit you wish to delete.")

    def __show_selected_visit_data(self):
        # Get the indexes of all selected cells
        selected_indexes = self.visits_tableView.selectedIndexes()
        if selected_indexes:
            # Get the row number of the first selected index
            selected_row = selected_indexes[0].row()
            ...
            visit = self.visit_service.get_visit(
                self.selected_visit)
            if visit:
                self.ui.year_of_visit_calendar.setDate(
                    QDate(visit.year_of_visit, 1, 1))
            ...
            if visit:
                ...
                self.endoscopy_pixmaps = self.endoscopy_file_service.retrieve_endoscopy_images_for_visit(
                    visit.visit_id)
    ...
    def __endoscopy_upload_button_clicked(self):
        ...
        for filename in filenames:
            ...
            if match:
                ...
                break
        if not error:
            self.ui.endoscopy_textfield.setText(
                str(len(filenames)) + " Files selected")
            for i in range(len(filenames)):
                ...
                endoscopy_file_dict = {
                    # Todo Button für Upload nur aktivieren, wenn ein Visit selektiert ist
                    'visit_id': self.selected_visit,
                    'image_position': positions[i],
                    # ToDo Filename langfristig besser nicht abspeichern
                    'filename': filenames[i],
                    'file': file_bytes
                }
                self.endoscopy_file_service.create_endoscopy_file(
                    endoscopy_file_dict)

    def __load_endoscopy_image(self):
        # Load and display the current image
        if 0 <= self.endoscopy_image_index < len(self.endoscopy_pixmaps):
            scaled_pixmap = self.endoscopy_pixmaps[self.endoscopy_image_index].scaledToWidth(
                200)
    ...
    def notify(self, text):
        ...
        msg.setIconPixmap(QtGui.QPixmap("./media/ok.png"))
        msg.exec()
